...
const TOTAL_CLIENTES = 2000000
const TOTAL_PEDIDOS = 10000000
...
function generateUniqueCustomerData(index: number): Cliente {
  const fullName = faker.name.findName()
  const email = faker.internet.email(fullName).toLowerCase()
  const address = faker.address.streetAddress()
  
  // Agregar timestamp y batch index para garantizar unicidad absoluta
  const uniqueEmail = `${Date.now()}_${index}_${email}`
  
  return { fullName, address, email: uniqueEmail }
}
...
async function insertCustomerData(): Promise<void> {
  ...
  for (const { startIndex, endIndex } of insertionRanges) {
    const batchCustomers = Array.from({ length: endIndex - startIndex }, (_, batchIndex) => 
      generateUniqueCustomerData(startIndex + batchIndex)
    )
    ...
    if ((endIndex) % 100000 === 0 || endIndex >= TOTAL_CLIENTES) {
...
async function insertOrderData(): Promise<void> {
  ...
  for (const { startIndex, endIndex } of insertionRanges) {
    ...
    if ((endIndex) % 100000 === 0 || endIndex >= TOTAL_PEDIDOS) {
...
async function main(): Promise<void> {
  try {
    ...
    console.log(`\n✅ Datos insertados exitosamente en ${duration.toFixed(2)} segundos`)
